...
exports.assignValues = catchAsync(async (req, res, next) => {
  ...
  await Promise.all(
    req.body.map(async (el) => {
      ...
      if (el.availableSize.price * 1 <= el.availableSize.discountPrice * 1) {
        return next(
          new AppError("Discount price must be lesser then price", 400)
        );
      }
      if (!checkProduct) {
...
exports.assignDataForCreateNewProduct = catchAsync(async (req, res, next) => {
  ...
  if (req.body.dealOfTheDay) {
    ...
    deal.dealOfTheDayExpires = new Date(new Date().setHours(23, 59, 59, 999));
  } else {
  ...
  if (!req.body.features || !req.body.features?.length)
    return next(new AppError("Atleast one features should be inclded.", 400));
  switch (req.body.productType) {
    case "single":
      if (req.body.discountPrice)
        if (req.body.discountPrice * 1 >= req.body.price * 1) {
          return next(
            new AppError("Discount price should be less then price.", 400)
          );
        }

      if (!req.body.bannerImage)
        return next(new AppError("Banner image should be included.", 400));
    ...
    case "colorOnly":
      ...
      let productDetails = await Promise.all(
        req.body.productDetails.map(async (el, index) => {
          ...
          if (!el.color)
            return next(new AppError("Color name should be included.", 400));
          el = {
            ...
            subDetails: [
              {
                ...
                discountPrice: el.subDetails[0].discountPrice ?? 0,
    ...
    case "sizeOnly":
      ...
      const subDetails = await Promise.all(
        req.body.productDetails[0].subDetails.map(async (el, index) => {
          if (!el.size)
            return next(new AppError("Size should be included.", 400));
          if (el.discountPrice)
            ...
            subDetails: [
              {
                price: el.subDetails[0].price,
                discountPrice: el.subDetails[0].discountPrice ?? 0,
                ecmpssId: el.subDetails[0].id ?? (await encryptID()),
              },
            ],
        ...
        productDetails: [
          {
            bannerImage: req.body.productDetails[0].bannerImage,
            imageGallery: req.body.productDetails[0].imageGallery,
            ecmpsId: req.body.productDetails[0].id ?? (await encryptID()),
            subDetails,
          },
        ],
        specification: req.body.specification,
...
exports.getAllProduct = catchAsync(async (req, res, next) => {
  ...
  if (!!req.query.category) {
    filterQuery.categorie = req.query.category;
...
exports.assignOrderProducts = catchAsync(async (req, res, next) => {
  ...
  const cart = await Promise.all(
    req.body.map(async (el) => {
      ...
      if (!product.active)
        return next(
          new AppError(`Product ${product.name} was out of stock.`, 400)
        );
      switch (product.productType) {
        ...
        case "colorOnly":
          ...
          const [colors] = await Promise.all([
            product.productDetails.find((els) => els.ecmpsId === el.color),
          ]);
              ...
              if (els.ecmpsId === el.color) {
                ...
                await Promise.all(
                  els.subDetails.map((els2) => {
                    if (els2.ecmpssId === el.size) size = true;
                  })
                );
              }
            }),
    ...
    carts.map(async (el) => {
      switch (el.product.productType) {
          ...
          await Promise.all(
            el.product.productDetails.map((els) => {
              if (els.ecmpsId === el.color) {
                ...
                colOn.discountPrice = els.subDetails[0].discountPrice;
                colOn.color = els.color;
              }
            })
          );
          if (!col) return next(new AppError("Product not found.", 400));
          el = {
            userAddress: address,
            productDetails: {
              productId: el.product._id,
        ...
        case "sizeOnly":
          ...
          await Promise.all(
            el.product.productDetails[0].subDetails.map((els) => {
              if (els.ecmpssId === el.size) {
                siz = true;
                sizOn.bannerImage = el.product.productDetails[0].bannerImage;
                sizOn.imageGallery = el.product.productDetails[0].imageGallery;
          ...
          if (!siz) return next(new AppError("Product not found.", 400));
          el = {
            userAddress: address,
            productDetails: {
              productId: el.product._id,
          ...
          await Promise.all([
            el.product.productDetails.map(async (els) => {
              if (els.ecmpsId === el.color) {
                ...
                await Promise.all(
                  els.subDetails.map((els2) => {
                    if (els2.ecmpssId === el.size) sizs = true;
                    colVals.price = els2.price;
                    colVals.discountPrice = els2.discountPrice;
...
exports.cancelAOrder = catchAsync(async (req, res, next) => {
  ...
  if (!order) {
    return next(
      new AppError("Something went wrong while cancelling your order.", 400)
    );
  }
  ...
  switch (product.productType) {
    ...
    case "colorOnly":
      ...
      let [colors] = await Promise.all([
        product.productDetails.find((els) => els.ecmpsId === req.body.color),
      ]);
    ...
    case "sizeOnly":
      ...
      if (!sizes) sizes = product.productDetails[0].subDetails[0].ecmpssId;
    ...
    case "colorWithSize":
      ...
      await Promise.all([
        product.productDetails.map(async (els) => {
          if (els.ecmpsId === req.body.color) {
            ...
            await Promise.all(
              els.subDetails.map((els2) => {
                if (els2.ecmpssId === req.body.size) size = true;
              })
            );
          }
        }),
      ...
      if (!color || !size) {
        ...
        req.body.size = product.productDetails[0].subDetails[0].ecmpssId;
      }
    ...
    carts.map(async (el) => {
      switch (el.product.productType) {
        case "single":
          ...
          discountPrice = discountPrice + el.discountPrice * el.quantity;
          finalPrice =
          ...
          break;
        case "colorOnly":
          if (!el.color) return next(new AppError("Invalid product!", 400));
          let color = false;
          await Promise.all(
            el.product.productDetails.map((els) => {
              if (els.ecmpsId === el.color) {
          ...
          if (!color) return next(new AppError("Invalid product!", 400));
          el = {
          ...
          discountPrice =
            discountPrice + color.subDetails[0].discountPrice * el.quantity;
          finalPrice =
            (!!color.subDetails[0].discountPrice * el.quantity
              ? color.subDetails[0].discountPrice * el.quantity
              : color.subDetails[0].price * el.quantity) + finalPrice;
          return el;
          break;
        case "sizeOnly":
          if (!el.size) return next(new AppError("Invalid product!", 400));
          let size = false;
          await Promise.all(
            el.product.productDetails[0].subDetails.map((els) => {
              if (els.ecmpssId === el.size) {
          ...
          if (!size) return next(new AppError("Invalid product!", 400));
          el = {
          ...
          discountPrice = discountPrice + size.discountPrice * el.quantity;
          finalPrice =
        ...
        case "colorWithSize":
          ...
          discountPrice =
            discountPrice + colVals.discountPrice * colVals.quantity;
          finalPrice =
...
exports.moveCartToCheckout = catchAsync(async (req, res, next) => {
  ...
  if (carts.length !== req.body.length) {
    return next(
      new AppError("Something went wrong while processing your request.", 400)
    );
  }
