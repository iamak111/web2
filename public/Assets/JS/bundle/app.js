...
const send_otpx = document.getElementById("send_otpx");
const verify_otpx = document.getElementById("verify_otpx");
...
if (send_otpx) {
  send_otpx.addEventListener("submit", (e) => {
    e.preventDefault();

    const name = document.getElementById("user_namex").value;
    const phone = document.getElementById("phone_numx").value;
    if (!phone) return alert("Please enter the phone number.");
    if (!name) return alert("Please enter the user name.");
    if (phone.length !== 10) return alert("Phone number shoule be 10 numbers.");
    return sendOtp({ phone, name }, "signup");
  });
}

if (verify_otpx) {
  verify_otpx.addEventListener("click", (e) => {
    e.preventDefault();
    const phone = document.getElementById("phone_numx").value;
    const otp = document.getElementById("verification_otpx").value;
...
if (update_address.length) {
  [...update_address].map((el, i) => {
    el.addEventListener("submit", (e) => {
      ...
      const landmark = document.getElementsByClassName("landmark")[i].value;
